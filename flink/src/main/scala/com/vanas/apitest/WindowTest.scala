    ...
    def main(args: Array[String]): Unit = {
        ...
        val dataStream = inputStream.map(data => {
            ...
            SensorReading(arr(0), arr(1).toLong, arr(2).toDouble)
                ...
                //.assignAscendingTimestamps(_.temperature * 1000L)//升序数据提取时间戳
        ...
        val resultStream = dataStream
                .map(data => (data.id, data.temperature, data.timestramp))
                .keyBy(_._1) //按照二元组的第一个元素id分组
                ...
                .timeWindow(Time.seconds(15)) //最常用
